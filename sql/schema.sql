...
-- ------------------------------------------------------------
-- COTIZACIONES — solicitudes de cotización armadas desde el export
-- de Capataz ("Venta y Compras → Movimientos → Gestión personalizada
-- de ventas y compras"), ver sql/013_cotizaciones.sql,
-- sql/014_cotizaciones_bloques.sql y sql/015_cotizaciones_confirmado.sql.
-- `bloque` es una etiqueta libre (ej. "Pañol"/"Despacho") que divide una
-- misma solicitud en grupos con su propia lista de proveedores invitados
-- cada uno; `confirmado` marca un ítem como compra ya decidida (se
-- oculta de la comparativa) — ver CLAUDE.md sección 9.
-- ------------------------------------------------------------
